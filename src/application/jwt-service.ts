...
export const jwtService = {
    async createJwtToken(foundUserInDb: UserDbType) {
        ...
        const accessToken = jwt.sign({userId: foundUserInDb.id}, settings.JWT_SECRET, {expiresIn: '10h'})
        const refreshToken = jwt.sign({userId: foundUserInDb.id}, settings.JWT_SECRET, {expiresIn: '1h'})
    ...
    async tryDecodeAndCreate(refreshToken: string) {
        ...
        if (!decodedRefreshToken) {
            ...
            if (checkRefreshTokenInDb) {
                ...
                const newAccessToken = jwt.sign({userId: decodedRefreshToken.userId}, settings.JWT_SECRET, {expiresIn: '10h'})
                const newRefreshToken = jwt.sign({userId: decodedRefreshToken.id}, settings.JWT_SECRET, {expiresIn: '1h'})
